...
void CollisionFilteringDemo::CreateObjects() {
	...
	float yPos = 0;
	for (int i = 0; i < 25; i++) {		
		float xPos = -5 + 2*(float)(i % 5);																		// Calculate x position for the object (5 columns)
		if (i % 5 == 0) yPos += 2;																				// Calculate y position for the object (5 rows)
		...
		CreateGameObject(new btSphereShape(1.2),
			...
			position,
			COLGROUP_SPHERE, COLGROUP_SPHERE | COLGROUP_STATIC);												// spheres can collide with the ground plane and other spheres
	}
}
