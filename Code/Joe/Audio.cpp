...
const int NUM_SOUND_FX = 5;			// Number of sound effects in array
...
std::string arrSoundEffects[NUM_SOUND_FX][3] = {
	...
	{ "SFX/Bonus.wav", "bonusFX", "Bonus Effect" },
...
/* 
	...
	Adds the tracks to the listOfMusic vector, picks a random song to play, and plays it

	If the file fails to load it displays an identifying error message, details in music array
	An ID for each track is not necessary as the song is selected at random	but, 
	is stored in case a certain part of the project requires a certain audio track
*/
...
bool Audio::LoadFX() {
	...
	if (pChunk == 0) {
		std::cout << "Could not load SFX: "<< id <<"ERROR - " << Mix_GetError() << std::endl;
	}
	else std::cout << "Loading SFX: " << id << std::endl;
	m_sfxs[id] = pChunk;
	*/
	bool success = true;
...
/*
	...
	Calls identifyTrack() to display the track details, outputs current song name to console
*/
...
/*
	Skips the current song that is playing backwards. If current song is the first song in the array, 
	it plays the song at the end of the listOfMusic array. identifyTrack() displays the track details
*/
void Audio::musicBack() {						// Pick previous track on the list
	//std::cout << "Current Song Index Before Decrement: " << currentSong << std::endl;
	if (currentSong > 0) currentSong--;
	else currentSong = listOfMusic.size() - 1;
	//std::cout << "Current Song Index After Decrement: " << currentSong << std::endl;
